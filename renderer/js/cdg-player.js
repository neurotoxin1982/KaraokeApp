/**
 * CDG Player — Web Audio API, sample-accurate sync.
 * In Electron: no autoplay restrictions, AudioContext starts immediately.
 */

const CDG_PACKET_SIZE     = 24;
const CDG_PACKETS_PER_SEC = 300; // 75 sectors/s × 4 packets/sector
const CDG_WIDTH  = 300;
const CDG_HEIGHT = 216;
const CDG_CMD    = 0x09;

let cdgSyncOffset = 0; // seconds, set by UI
...
class CdgPlayer {
  constructor(canvas) {
    ...
    this._bgColor = 0;
    this._data = null; this._pktIdx = 0;
    this._rafId = null; this._stopped = false; this._paused = false;
    this._audioCtx = null; this._startAt = 0; this._outputLatency = 0;
    this._follower = false; this._getPos = null;
  }
  ...
  /** Mode 1: wall-clock sync (main window) — immune to AudioContext timing quirks */
  start(audioCtx, startAt) {
    // Compute the wall-clock ms at which audio position = 0
    const msUntilStart = (startAt - audioCtx.currentTime) * 1000;
    this._wallMs   = performance.now() + msUntilStart;
    this._pausedPos = 0;
  ...
  /** Mode 2: Follower (popout window receives position from BC) */
  startFollower(getPosFn) {
    this._follower = true; this._getPos = getPosFn;
    this._stopped = false; this._paused = false;
    if (!this._rafId) this._renderLoop();
  }

  pause() {
    this._pausedPos = Math.max(0, (performance.now() - this._wallMs) / 1000);
    this._paused = true;
  }

  resume(audioCtx, startAt) {
    if (!this._follower) {
      // Wall time when audio pos = 0: now minus how far we are into the song
      this._wallMs = performance.now() - this._pausedPos * 1000;
    }
    this._paused = false;
    if (!this._rafId) this._renderLoop();
  }
  ...
  // wallWhenMs: performance.now() value at which audio will be at position pos
  seekTo(pos, wallWhenMs) {
    const ref = wallWhenMs != null ? wallWhenMs : performance.now();
    this._wallMs = ref - pos * 1000;
    this._hardReset();
  ...
  _hardReset() {
    this._pktIdx = 0; this._pixels.fill(0); this._clut.fill(0); this._bgColor = 0;
  }

  _position() {
    if (this._follower && this._getPos) return Math.max(0, this._getPos() + cdgSyncOffset);
    if (this._paused) return Math.max(0, this._pausedPos + cdgSyncOffset);
    if (this._wallMs == null) return 0;
    return Math.max(0, (performance.now() - this._wallMs) / 1000 + cdgSyncOffset);
  }
  ...
  _renderLoop() {
    ...
    const target = Math.floor(pos * CDG_PACKETS_PER_SEC);
    if (target < this._pktIdx - 2) this._hardReset();
    while (this._pktIdx <= target) {
      const off = this._pktIdx * CDG_PACKET_SIZE;
      if (off + CDG_PACKET_SIZE > this._data.length) break;
      this._processPacket(off); this._pktIdx++;
    }
    this._paint();
  }
  ...
  _borderPreset(data) {
    const c=data[0]&0x0F;
    for(let y=0;y<CDG_HEIGHT;y++) for(let x=0;x<CDG_WIDTH;x++)
      if(x<6||x>=CDG_WIDTH-6||y<12||y>=CDG_HEIGHT-12) this._pixels[y*CDG_WIDTH+x]=c;
  }
  ...
  _tile(data, xor) {
    ...
    for(let r=0;r<12;r++){
      ...
      for(let c=0;c<6;c++){
        const bit=(row>>(5-c))&1, idx=(ty+r)*CDG_WIDTH+(tx+c);
  ...
  _scroll(data, copy) {
    ...
    let dx=0,dy=0;
    if((h>>4)===1)dx=h&7; if((h>>4)===2)dx=-(CDG_WIDTH-(h&7));
    if((v>>4)===1)dy=v&15;if((v>>4)===2)dy=-(CDG_HEIGHT-(v&15));
